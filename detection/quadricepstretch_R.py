    ...
    async def receive(self, text_data):
        ...
        if frame_data:
            ...
            if results.pose_landmarks:
                ...
                left_knee_angle = u.calculateAngle(landmarks[point[23]],landmarks[point[25]],landmarks[point[27]])
                
                distance_camera = u.calculateDistance(landmarks[point[12]],landmarks[point[24]])      
                color2 = (0, 0, 255)
                cv2.putText(frame, f'{int(left_knee_angle)}', (int(landmarks[point[25]][0]), int(landmarks[point[25]][1])), cv2.FONT_HERSHEY_PLAIN, 2, color2, 3)
                ...
                if distance_camera >= 600:
                    ...
                    if 15 <= left_knee_angle <= 60:
                        label = 'Correct pose'
                        color = (0, 255, 0)
                            
                    elif left_knee_angle > 150:
                        label = 'Lift your right leg backward !'
                        color = (0, 0, 255)
                    elif 60 < left_knee_angle <= 150: 
                ...
                x1 = int(landmarks[point[23]][0])
                y1 = int(landmarks[point[23]][1])
                x2 = int(landmarks[point[25]][0])
                y2 = int(landmarks[point[25]][1])
                x3 = int(landmarks[point[27]][0])
                y3 = int(landmarks[point[27]][1])
