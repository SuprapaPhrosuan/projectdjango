    ...
    async def receive(self, text_data):
        ...
        if frame_data:
            ...
            if results.pose_landmarks:
                ...
                right_shoulder_angle = u.calculateAngle(landmarks[point[14]],landmarks[point[12]],landmarks[point[24]]) 
                right_elbow_angle = u.calculateAngle2(landmarks[point[16]],landmarks[point[14]],landmarks[point[12]])
                ...
                cv2.putText(frame, f'{int(right_elbow_angle)}', (int(landmarks[point[14]][0]), int(landmarks[point[14]][1])), cv2.FONT_HERSHEY_PLAIN, 2, color2, 3)
                ...
                if distance_camera >= 200:
                    ...
                    if  (65 <= right_shoulder_angle <= 100) and (160 <= right_elbow_angle <= 200): 
                        label = 'Correct pose'
                        color = (0, 255, 0)
                    elif (right_shoulder_angle < 65 or 100 < right_shoulder_angle <= 360) and (160 <= right_elbow_angle <= 200):
                        label = 'Stretch L-arm to R-side more !' 
                        color = (0, 0, 255)

                    elif (right_shoulder_angle < 65 and right_elbow_angle < 160) or (right_shoulder_angle <= 360 and right_elbow_angle <= 360):
                        label = 'Do not bend L-elbow !'
                        color = (0, 0, 255)
                    else:
                        label = 'Unknown' 
                        color = (0, 0, 255)
